// Generates the human-editable import templates, populated from the current DB
// (which was seeded from the original Excels + synced against the course API).
//
//   npm run export:templates            -> ../plantillas/PLANES.xlsx + ELECTIVAS.xlsx
//   TEMPLATES_DIR=/some/dir npm run export:templates
//
// One row = one course/slot. `Semestre` is an explicit column (never inferred).
...
async function buildPlanes(): Promise<ExcelJS.Workbook> {
  const wb = new ExcelJS.Workbook();
  wb.creator = "Pensum Interactivo — export:templates";
  wb.created = new Date();

  // --- one sheet per catalog ---
  ...
  for (const slug of CATALOG_ORDER) {
    ...
    for (const sem of semesters) {
      ...
      // informational subtotal (Código empty => the parser skips it)
    ...
    // missing "Semestre" -> red highlight (helps future edits)
  ...
  // --- _REQUISITOS_GRADO (attestations + gates; identical across catalogs) ---
  ...
  for (const n of nodes) {
    if (seenKey.has(n.key)) continue; // same key across catalogs — one row
    seenKey.add(n.key);
    wsNode.addRow({
      key: n.key,
      label: n.label,
...
async function buildElectivas(): Promise<ExcelJS.Workbook> {
  const wb = new ExcelJS.Workbook();
  wb.creator = "Pensum Interactivo — export:templates";
  wb.created = new Date();
